...
int main(int argc, char **argv)
{
    ...
    for ( i = 0; i < iterations; i++ ) {
        ...
        for ( j = 0; j < numrows; j++ ) {
            thread_stats[i][j] = malloc(NUM_DATA_POINTS*sizeof(unsigned int));
            if ( !thread_stats[i][j] ) {
                fprintf(stderr, "%s: Allocation error\n", __FUNCTION__);
                exit(EXIT_FAILURE);
            }
    ...
    // Main thread takes a snapshot of the progress every that many edges :)
    interval = el->nedges / NUM_DATA_POINTS;
    // Stores running number of cycles found by each thread ;)
    cycles_so_far = malloc(maxthreads*sizeof(unsigned int));
    if ( !cycles_so_far ) {
        fprintf(stderr, "%s: Allocation error\n", __FUNCTION__);
        exit(EXIT_FAILURE);
    }
