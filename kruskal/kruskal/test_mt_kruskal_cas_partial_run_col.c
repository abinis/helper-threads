    ...
    for ( iter = 0; iter < iterations; iter++ ) {
        ...
        for ( nthreads = 1; nthreads <= maxthreads; nthreads++ ) {
            ...
            for ( datapoint = 0; datapoint < numdatapoints; datapoint++ ) {
                ...
                for ( e = 0; e < until; e++ ){
                    if ( el->edge_array[e].edge_color == MSF_EDGE ) {
                        msf_weight += el->edge_array[e].weight;
                        msf_edge_count++;
                    } else if ( el->edge_array[e].edge_color == CYCLE_EDGE_MAIN ) {
                        cycles_main++;
                        cycles_helper[el->edge_array[e].edge_color-1]++;
                    }
                    else {
                        cycles_helper[el->edge_array[e].edge_color-1]++;
                        ht_total_cycles++;
