    ...
    for ( iter = 0; iter < iterations; iter++ ) {
        ...
        for ( nthreads = 1; nthreads <= maxthreads; nthreads++ ) {
            ...
            for ( datapoint = 0; datapoint < numdatapoints; datapoint++ ) {
                ...
                printf("\n");
                edge_color_bitmask_print(edge_color_main);

                for ( e = 0; e < until; e++ ){
                    if ( edge_color_bitmask_get_color(edge_color_main,e) == 0 ) {
                        msf_weight += el->edge_array[e].weight;
                        msf_edge_count++;
                    } else if ( edge_color_bitmask_get_color(edge_color_main,e) == 1 ) {
                        // should be 'total_cycles', but ok
                        cycles_main++;
