...
tsctimer_t tim, tim2, tim3, tim4, tim5, tim6, tim7;
...
int *edge_color_main;
int *loopcnt;
//char *edge_color_helper; !!!!!! comment out  edge_color_helper  !!!!!!

int nthreads;

void touch_structures2(edgelist_t *_el,
                             adjlist_t *_al,
...
int main(int argc, char **argv)
{
    ...
    loopcnt = malloc((maxthreads+1)*sizeof(int));
    ...
    for ( nthreads = 1; nthreads <= maxthreads; nthreads++ ) {
        ...
        kruskal_helper_init3(el, &edge_color_main);
        ...
        fprintf(stdout, "cycles for all loop:%lf  ", timer_total(&tim4));

        fprintf(stdout, "cycles for find_set:%lf  ", timer_total(&tim2));

        fprintf(stdout, "cycles for union:%lf  ", timer_total(&tim3));

        fprintf(stdout, "cycles for find_set_helper:%lf  ", timer_total(&tim));
        ...
        kruskal_helper_destroy3(edge_color_main);
       // if (nthreads == 1) {
            //for(e=0; e < el->nedges; e++) {
                //if (edge_color_main[e] == MSF_EDGE)
                   // edge_color_main[e] = 0;
