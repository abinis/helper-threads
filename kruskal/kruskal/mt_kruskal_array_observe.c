...
extern volatile unsigned int *cycles_so_far;
extern unsigned int ***thread_stats;
unsigned int interval;
extern struct timespec time_interval;
...
extern volatile unsigned int cycles_main;
...
void *kruskal_ht(void *args)
{
    ...
    *why = CYCLE_FOUND_BY_MAIN;
    ...
    // Code for the Observer Thread
    if ( thread_type == OBSERVER_THR ) {

        pthread_barrier_wait(&bar);

        // loop until main finishes
        while ( !main_finished ) {
