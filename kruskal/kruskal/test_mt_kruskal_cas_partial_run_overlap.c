    ...
    for ( iter = 0; iter < iterations; iter++ ) {
        ...
        for ( nthreads = 1; nthreads <= maxthreads; nthreads++ ) {
            ...
            for ( datapoint = 0; datapoint < numdatapoints; datapoint++ ) {
                ...
                int divided_by = ( nthreads > 1 ) ? nthreads-1 : nthreads;
                int chunk_size = until / divided_by;
                ...
                for ( i = 0; i < nthreads; i++ ) {
                    if ( i < 1 ) {
                        ...
                        targs[i].end = ( i <= rem ) ? (i-1)*(chunk_size+1) : rem*(chunk_size+1)+(i-1-rem)*chunk_size;
                        //plusone = ( rem-- > 0 );
                        int plusone = ( i <= rem );
                ...
                //assert( msf_edge_count+cycles_main+ht_total_cycles == el->nedges );
